...
const _head_satellites_sel =
  ...
  providers_dict[Providers.HEADAEROSPACE]
    .map((constellation) => constellation_dict[constellation].satellites.map((sat) => head_search_names[sat]))
    .flat()
    .filter(unique)
    .join('$') +
  '$';
...
const search_head = async (search_settings, apikey = '', searchPolygon = null, setters = null) => {
  ...
  if (!res_text.includes('ERROR: APP failed')) {
    const payload_str = res_text.substring(res_text.indexOf('jsonscenelist=') + 14, res_text.lastIndexOf(']') + 1);
  ...
  const head_results = {
    ...
    features: head_results_raw.map((r) => {
      const feature = {
        geometry: {
          type: 'Polygon',
          coordinates: [r.footprintlon.slice(1).map((lon, idx) => [lon, r.footprintlat[1 + idx]])],
        },
        properties: {
