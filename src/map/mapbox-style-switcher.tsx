...
function MapboxStyleSwitcher(props) {
  ...
  .map(style_id => ({
    style_id, 
    url: `https://api.mapbox.com/styles/v1/mapbox/${style_id}/static`
          + `/${map.getCenter().lng},${map.getCenter().lat},${Math.max(1, map.getZoom() - 2)},${bearing},${pitch}`
          + `/${width}x${height}@2x?access_token=${props.mapboxAccessToken}&attribution=false` // {@2x}
  })
  )
  return (
      (!styleSwitcherOpen) ? 
